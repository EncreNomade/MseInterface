...
})(mse);var GameSimonComa = function() {
    mse.Game.call(this);
...
extend(GameSimonComa, mse.Game);var Fourchelangue = function() {
    ...
    var base = [
        ...
        {fr:2,x:115,y:388,head:2,color:"#393e7f"},
    ...
    var levelupbn = new mse.Button(null, {pos:[247,380],size:[105,35],font:'12px Arial',fillStyle:'#FFF'}, 'Je me chanllage', 'aideBar');
    ...
    var RatBase = function(id, wordid, currfcl) {
        ...
        this.draw = function(ctx){
            ...
            if(this.cri) {
                ...
                ctx.textAlign = "center";
                ctx.textBaseline = "middle";
                ctx.font = "blod 22px Arial";
                ctx.fillText(this.currfcl[this.wordid], ratPos.text.x, ratPos.text.y);
            }
            else ratHead.drawFrame(this.base.head, ctx, ratPos.head.x, ratPos.head.y);
    ...
    this.init = function(levelup) {
        ...
        if(levelup === true && this.mode < 2) this.mode++;
    ...
    this.clicked = function(e) {
        ...
        if(this.state == "FAIL" && replaybn.inObj(x, y)) {
            this.init();
        }
        ...
        // Finish game
            ...
            for(var i in this.bases) {
                ...
                if(this.bases[i].inObj(x, y)) {
                    ...
                    var keycount = this.lines[this.currGuess.line].keywords.length;
                    // Line finished
                    if(this.currGuess.index >= keycount) {
                        this.currGuess.line++;
                        this.currGuess.index = 0;
                        keycount = this.lines[this.currGuess.line].keywords.length;
                    }
                    // Register guessed word
                    if(this.currGuess.line <= this.lines.length-1 && this.currGuess.index <= keycount-1)
                        this.lines[this.currGuess.line].keywords[this.currGuess.index].guess = this.currfcl[this.bases[i].wordid];
                    // Finishing guess
                    if(this.currGuess.line >= this.lines.length-1 && this.currGuess.index >= keycount-1) {
                        this.check();
                    }
                    this.currGuess.index++;
                    return;
                }
            }
        }
    ...
    this.draw = function(ctx) {
        ...
        if(this.currTime <= 2.5) {
            ctx.fillText("Mémorise vite l'ordre des mots colorés", 0,-40);
            ctx.fillText("et les paroles des rats !", 0,0);
        }
...
reaction.startratflash=function(){ animes.ratflash.start(); };
action.startratflash.register(reaction.startratflash);
action.addTextEffect=action.couvshow;
reaction.addTextEffect=function(){ function textEffect(effet,obj) {
...
action.addTextEffect.register(reaction.addTextEffect);mse.currTimeline.start();};mse.autoFitToWindow(createbook);