...
9. mobile_data - returns an array of data about mobiles. Note the browser/os number data is very
   ...
   listed by array index number: 
     ...
     8 - $mobile_tablet (test if not null rather than t/f, returns for example: ipad, android 3)
         Note that this is barebones, but will catch 98+% of current tablets at this point.
...
18. html_type - returns following values, numeric rather than t/f, so you can use 
...
19. engine_data - Returns array of layout engine data, if present. Opera>=10;MSIE>=7;Gecko;Webkit
...
function browser_detection( $which_test, $test_excludes='', $external_ua_string='' ) 
{
	...
	*/
	if ( !$b_repeat ) {
		...
		*/
		$a_gecko_types = array( 'bonecho', 'camino', 'epiphany', 'firebird', 'flock', 'galeon', 'iceape', 'icecat', 'k-meleon', 'minimo', 'multizilla', 'phoenix', 'songbird', 'swiftfox', 'seamonkey', 'shadowfox', 'shiretoko', 'iceweasel', 'firefox', 'minefield', 'netscape6', 'netscape', 'rv' );
		...
		*/
		$a_webkit_types = array( 'arora', 'chromium', 'chrome', 'epiphany', 'gtklauncher', 'icab', 'konqueror', 'maxthon',  'midori', 'omniweb', 'rekonq', 'safari', 'shiira', 'uzbl', 'applewebkit', 'webkit' );
		...
		if ( !is_array( $os_working_data ) 
			...
			switch ( $os_working_type ) {
				...
				case 'nt':
					...
					// windows xp
					elseif ( strstr( $pv_browser_string, 'nt 5.1' ) 
						|| strstr( $pv_browser_string, 'xp' ) ) {
						$os_working_number = 5.1;//
					}
					// windows 2000
					elseif ( strstr( $pv_browser_string, 'nt 5' ) 
						|| strstr( $pv_browser_string, '2000' ) ) {
...
function get_mobile_data( $pv_browser_user_agent )
{
	...
	*/
	...
	// mobile/tablet
	$a_mobile_tablet = array( 'ipad', 'android 3' );
...
function getBrowser() {
    ...
    $browser        =   array("name" => "Unknown Browser", "mobile" => false);
